...
from data_process import get_processed_datas, Dataset2, upsample, Test_Dataset
...
list_root = '../lists/face/training.txt'
...
def train():
    for epoch in range(1, epoches+1):
        ...
        for ii, (img, label) in enumerate(dataloader):
            ...
            # loss2 = recon_loss(pose, img, label)
            loss.backward()
            optimizer1.step()
...
if __name__ == '__main__':
    ...
    # model2.load_state_dict(torch.load(path+'epoch2-44.pth'))
    ...
    scheduler = optim.lr_scheduler.MultiStepLR(optimizer1, milestones=[45,57,74], gamma=0.1)
    ...
    test_dataset = Dataset2(test_imgs,test_labels,transforms=test_transform)
